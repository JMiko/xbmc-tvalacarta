...
logger.debug("[megaporn.py] Cookiefile="+COOKIEFILE) 
...
# To activate it just call Megavideo(url) - where url is your megaporn url.
def getcode(mega):
    xbmc.output("[megaporn.py] mega="+mega)
    if mega.startswith('http://www.megaporn.com/?v='):
        mega = mega[-8:]
    xbmc.output("[megaporn.py] mega="+mega)
    return mega
...
def geturl(mega):
    ...
    xbmc.output("[megaporn.py] Megaporn")
    try:
        modoPremium = config.getSetting("megapornpremium")
    except:
        modoPremium = "false"
    xbmc.output("[megaporn.py] modoPremium="+modoPremium)
    ...
    xbmc.output("[megaporn.py] movielink="+movielink)
        
    return movielink
#####END of part 2

def getlowurl(code):
    xbmc.output("[megaporn.py] Baja calidad")
    ...
    if modoPremium == "false":
        xbmc.output("[megaporn.py] usando modo normal para baja calidad")
        req = urllib2.Request("http://www.megaporn.com/video/xml/videolink.php?v="+code)
        req.add_header('User-Agent', 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.8.1.14) Gecko/20080404 Firefox/2.0.0.14')
        req.add_header('Referer', 'http://www.megaporn.com/')
        ...
        if len(errort) <= 0:
            ...
            if quality == "1":
                ...
                if len(hd)>0 and hd[0]=="1":
                    ...
                    movielink = "http://www" + s[0] + ".megaporn.com/files/" + decrypt(un[0], k1[0], k2[0]) + "/?.flv"
                    return movielink        
        ...
        xbmc.output("[megaporn.py] usando modo premium para baja calidad")
        try:
        ...
        req = urllib2.Request("http://www.megaporn.com/video/xml/videolink.php?v="+code+"&u="+megaporncookie)
        req.add_header('User-Agent', 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.8.1.14) Gecko/20080404 Firefox/2.0.0.14')
        req.add_header('Referer', 'http://www.megaporn.com/')
        page = urllib2.urlopen(req);response=page.read();page.close()
        errort = re.compile(' errortext="(.+?)"').findall(response)
...
def GetMegapornUser(login, password):
    ...
    #print "-------------------------------------------------------"
    url="http://www.megaporn.com/?c=login"
    #print url
    #print "-------------------------------------------------------"
    theurl = url
    ...
    txheaders =  {'User-Agent':'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.9.0.3) Gecko/2008092417 Firefox/3.0.3','Referer':'http://www.megaporn.com/?s=signup'}
    ...
    if len(matches)==0:
        xbmc.output("No se ha encontrado la cookie de Megaporn")
        xbmc.output("----------------------")
        xbmc.output("Respuesta de Megaporn")
        xbmc.output("----------------------")
        xbmc.output(data)
        xbmc.output("----------------------")
        xbmc.output("----------------------")
