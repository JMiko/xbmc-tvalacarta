...
xbmcplugin.setPluginFanart(int( sys.argv[ 1 ] ), fanartfile)
...
def mainlist(params,url,category):
    ...
    xbmctools.addnewfolder( CHANNELNAME , "ListaDetallada" , "Cine" , config.get_localized_string(30902) , "http://tvshack.cc/movies" , thumbnail="" , plot="" )
...
#    text1 = 'Leyendo %s...' %(category,) #'Leyendo %s...'
    ...
    if totalseries == 0:
        ...
        FinalizaPlugin_con_error(category)
        return
    i = 0
...
def FinalizaPlugin (category):
    ...
    xbmcplugin.setContent (int( sys.argv[ 1 ] ) , category) #Estamos usando category como content.
...
def FinalizaPlugin_con_error (category):
    """Tareas comunes al final del plugin. Sin ordenaci�n
    """
    # Indicar metadatos del plugin para skis (Categor�a y contenido)
