#------------------------------------------------------------
# tvalacarta launcher for dreambox
...
config.plugins.tvalacarta = ConfigSubsection()
config.plugins.tvalacarta.storagepath = ConfigText(default="/media/hdd", fixed_size=False)
config.plugins.tvalacarta.imagecache = ConfigEnableDisable(default=True)
config.plugins.tvalacarta.imagescaling = ConfigSelection(default="1", choices = [("0", _("simple")), ("1", _("better"))])
config.plugins.tvalacarta.imagescaler = ConfigSelection(default="0", choices = [("0", _("decodePic()")), ("1", _("getThumbnail()"))])
config.plugins.tvalacarta.showadultcontent = ConfigYesNo(default=False)
config.plugins.tvalacarta.showsecretcontent = ConfigYesNo(default=False)
config.plugins.tvalacarta.version = NoSave(ConfigText(default="282"))

default = config.plugins.tvalacarta.storagepath.value + "/tvalacarta/movies"
tmp = config.movielist.videodirs.value
if default not in tmp:
    tmp.append(default)
config.plugins.tvalacarta.moviedir = ConfigSelection(default=default, choices=tmp)
...
class downloadJob(Job):
    def __init__(self, toolbox, cmdline, filename, filetitle):
        print "[tvalacarta] downloadJob.__init__"
...
class downloadTask(Task):
    ...
    def __init__(self, job, cmdline, filename, filetitle):
        print "[tvalacarta] downloadTask.__init__"
...
class TvalacartaMoviePlayer(MoviePlayer):
    def __init__(self, session, service, movieinfo=None):
        print "[tvalacarta] TvalacartaMoviePlayer.__init__"
    ...
    def leavePlayer(self):
        print "[tvalacarta] TvalacartaMoviePlayer.leavePlayer"
        self.leavePlayerConfirmed(True)

    def leavePlayerConfirmed(self, answer):
        print "[tvalacarta] TvalacartaMoviePlayer.leavePlayerConfirmed"
        if answer:
            self.close()

    def doEofInternal(self, playing):
        print "[tvalacarta] TvalacartaMoviePlayer.doEofInternal"
        self.leavePlayerConfirmed(True)

    def showMovies(self):
        print "[tvalacarta] TvalacartaMoviePlayer.showMovies"
        pass
...
class Tvalacarta(Screen):
    def __init__(self, session, feedurl="http://www.dreambox-plugins.de/feeds/mediathek/main.xml", feedtitle="Canales", feedtext=""):
        ...
        self.skin = "<screen position=\"0,0\" size=\"" + str(size_w) + "," + str(size_h) + "\" flags=\"wfNoBorder\" title=\"tvalacarta\"> \
            <ePixmap name=\"dp_logo\" position=\"50,30\" zPosition=\"2\" size=\"250,200\" pixmap=\"/usr/lib/enigma2/python/Plugins/Extensions/tvalacarta/images/logotv.jpg\" /> \
        ...
        self.picload.setPara((self.picX-10, self.picY-(self.textsize*2), sc[0], sc[1], config.plugins.tvalacarta.imagecache.value, int(config.plugins.tvalacarta.imagescaling.value), "#00000000"))
    ...
    def loadFrame(self):
        ...
        if self.feedtitle == "Bookmarks":
            self.getBookmarks()
        elif self.feedurl.startswith("tvalacarta"):
            self.gettvalacartaitems()
        else:
            self.getxmlfeed()

    def createMediaFolders(self):
        print "[tvalacarta] createMediaFolders"
        # Check if cache folder is on a mountable device and not inside flash-memory
        tmppath = config.plugins.tvalacarta.storagepath.value
        ...
        os_system("mkdir -p "+config.plugins.tvalacarta.storagepath.value+"/tvalacarta")
        os_system("mkdir -p "+config.plugins.tvalacarta.storagepath.value+"/tvalacarta/images")
        os_system("mkdir -p "+config.plugins.tvalacarta.storagepath.value+"/tvalacarta/movies")
        os_system("mkdir -p "+config.plugins.tvalacarta.storagepath.value+"/tvalacarta/tmp")

        # Check if Cache Folders were created successfully
        if not os_path.exists(config.plugins.tvalacarta.storagepath.value+"/tvalacarta"):
    ...
    def checkforupdate(self):
        try:
            ...
            print "[tvalacarta]: Could not download HTTP Page\n" + str(error)
    ...
    def gotUpdateInfo(self, html):
        ...
        if config.plugins.tvalacarta.version.value < remoteversion:
    ...
    def finishedPluginUpdate(self,retval):
        self.session.openWithCallback(self.restartGUI, MessageBox,_("tvalacarta plugin successfully updated!\nDo you want to restart Enigma2 GUI now?"), MessageBox.TYPE_YESNO)
    ...
    def getxmlfeed(self):
        print "[tvalacarta] getxmlfeed"
        feedurl = self.feedurl
        if self.feedtitle == "Startseite" and (config.plugins.tvalacarta.showadultcontent.value == True or config.plugins.tvalacarta.showsecretcontent.value == True):
            feedurl = feedurl + "?"
            if config.plugins.tvalacarta.showadultcontent.value:
                feedurl = feedurl + "&showadult=1"
            if config.plugins.tvalacarta.showsecretcontent.value:
    ...
    def gettvalacartaitems(self):
        print "[tvalacarta] gettvalacartaitems"
        '''
        ...
        if accion=="play":
            try:
                exec "from tvalacarta.channels import "+canal
        ...
        for item in itemlista:
            if item.action=="__movieplay":
                ...
                imgurl = ""
                url = "tvalacarta" + "|" + canal + "|" + item.action + "|" + item.url + "|" + item.server
    ...
    def gotxmlfeed(self, page=""):
        ...
        # Anade los canales de tvalacarta
        from core.item import Item
        ...
        for item in itemlist:
            if item.type=="generic":
                type = "cat"
                name = item.title
                imgurl = "http://tvalacarta.mimediacenter.info/dreambox/"+item.channel+".png"
                url = "tvalacarta|"+item.channel+"|mainlist|none|"
    ...
    def getBookmarks(self):
        ...
        bookmarkfile = "/etc/enigma2/tvalacarta.bookmarks"
    ...
    def getThumbnail(self):
        ...
        self.thumbfile = config.plugins.tvalacarta.storagepath.value+"/tvalacarta/images/"+str(self.Thumbnaillist[self.thumbcount][3])
    ...
    def gotThumbnail(self, data=""):
        if config.plugins.tvalacarta.imagescaler.value == "0":
    ...
    def newPage(self):
        ...
        for x in self.itemlist:
            ...
            if x[2] == self.currPage:
                ...
                self["thumb"+str(x[1])].instance.setPixmapFromFile("/usr/lib/enigma2/python/Plugins/Extensions/tvalacarta/images/item.png")
    ...
    def selectBookmark(self):
        self.session.open(Tvalacarta, self.itemlist[self.index][5], "Bookmarks", "Bookmarks from your favorite Movies")
    ...
    def key_menu(self):
        if self.feedtitle == "Startseite":
            self.session.openWithCallback(self.loadFrame, Tvalacarta_Settings)
        elif self.itemlist:
            self.session.openWithCallback(self.loadFrame, Tvalacarta_MenuOptions, self.itemlist[self.index])
    ...
    def key_ok(self):
        ...
        if self.itemlist[self.index][6] == "cat":
            self.session.open(Tvalacarta, self.itemlist[self.index][5], self.itemlist[self.index][3], self.feedtext + " - " + self.itemlist[self.index][3])
    ...
    def SendSearchQuery(self, query):
        if query is not None:
            ...
            self.session.open(Tvalacarta, searchurl, self.itemlist[self.index][3], self.feedtext + " - " + self.itemlist[self.index][3] + ": " + query)
    ...
    def Exit(self):
        if self.feedtitle == "Startseite":
            ...
            # Clean TEMP Folder
            if os_path.isdir(config.plugins.tvalacarta.storagepath.value+"/tvalacarta/tmp"):
                for filename in os_listdir(config.plugins.tvalacarta.storagepath.value+"/tvalacarta/tmp"):
                    filelocation = "%s/tvalacarta/tmp/%s" % (config.plugins.tvalacarta.storagepath.value,filename)
                    self.BgFileEraser.erase(filelocation)

            # Clean Image Cache
            if os_path.isdir(config.plugins.tvalacarta.storagepath.value+"/tvalacarta/images"):
                for filename in os_listdir(config.plugins.tvalacarta.storagepath.value+"/tvalacarta/images"):
                    filelocation = "%s/tvalacarta/images/%s" % (config.plugins.tvalacarta.storagepath.value,filename)
...
class MovieInfoScreen(Screen):
    def __init__(self, session, movieurl):
        print "[tvalacarta] MovieInfoScreen.__init__(movieurl="+movieurl+")"
        Screen.__init__(self, session)
        ...
        self.moviefolder = config.plugins.tvalacarta.moviedir.value
        self.imagefolder = config.plugins.tvalacarta.storagepath.value+"/tvalacarta/images"
        ...
        self.picload.setPara((270, 350, sc[0], sc[1], config.plugins.tvalacarta.imagecache.value, int(config.plugins.tvalacarta.imagescaling.value), "#00000000"))
    ...
    def keyBlue(self):
        if self.movieinfo is not None:
            os_system("echo 'movie:::%s:::%s:::%s\n' >> /etc/enigma2/tvalacarta.bookmarks" % (self.movieinfo[0], self.url, self.movieinfo[3]))
            self.session.open(MessageBox, _("Bookmark added!"), MessageBox.TYPE_INFO, timeout=5)

    def GetMovieInfo(self):
        print "[tvalacarta] GetMovieInfo(self.url="+self.url+")"
        try:
            if '-->' in self.url:
                # Request to download external page
    ...
    def GotMovieInfo(self, html):
        ...
        try:
            ...
            print "[tvalacarta] Getting UserAgent String failed. Using default ..."
    ...
    def GetMovieList(self):
        ...
        try:
            ...
            print "[tvalacarta] Could not download Movie-List\n%s" % (error)
    ...
    def GotMovieList(self, html):
        print "[tvalacarta] GotMovieList(html="+html+")"
        content = html.split("\n")
        entrylist = []
        filecount = 0
    ...
    def movieSelectCallback(self, movieinfo):
        ...
        if movieinfo is not None:
            ...
            if self.action == "cachedplayback":
                ...
                self.session.openWithCallback(self.MoviePlayerCallback, TvalacartaMoviePlayer, sref, movieinfo)
...
class PlayMovie(Screen):
    ...
    def __init__(self, session, movieinfo, movietitle, useragent):
        ...
        self.movieinfo = movieinfo
        self.destination = config.plugins.tvalacarta.storagepath.value+"/tvalacarta/tmp/"
        ...
        self["label_destination"] = StaticText("Destination: %s" % (config.plugins.tvalacarta.storagepath.value))
    ...
    def copyfile(self):
        ...
        print "[tvalacarta] execute command: " + cmd
    ...
    def playfile(self):
        if self.lastlocalsize > 0:
            ...
            self.session.openWithCallback(self.MoviePlayerCallback, TvalacartaMoviePlayer, sref, self.movieinfo)
        else:
...
class Tvalacarta_MenuOptions(Screen):
    def __init__(self, session, movieinfo):
        Screen.__init__(self, session)

        self.skin = """
            <screen position="center,center" size="400,200" title="tvalacarta - Menu Options">
        ...
        list.append(("tvalacarta Settings", "settingsmenu", "menu_settings", "50"))
    ...
    def okbuttonClick(self):
        ...
        if selection:
            if selection[1] == "addbookmark":
                os_system("echo '%s:::%s:::%s:::%s' >> /etc/enigma2/tvalacarta.bookmarks" % (self.movieinfo[6], self.movieinfo[3], self.movieinfo[5], self.movieinfo[4]))
                self.Exit()
            if selection[1] == "delbookmark":
                ...
                self.session.open(Tvalacarta, self.movieinfo[5], "Bookmarks", "Bookmarks from your favorite Movies")
            elif selection[1] == "viewdownloads":
                self.session.openWithCallback(self.Exit, Tvalacarta_TaskViewer)
            elif selection[1] == "settingsmenu":
                self.session.openWithCallback(self.Exit, Tvalacarta_Settings)
            else:
                self.Exit()
        else:
            self.Exit()
...
class Tvalacarta_TaskViewer(Screen):
    def __init__(self, session):
        Screen.__init__(self, session)
        self.session = session
        
        self.skin = """
            <screen name="MediathekTasksScreen" position="center,center" size="700,550" title="tvalacarta - Active Downloads">
...
class Tvalacarta_Settings(Screen, ConfigListScreen):
    skin = """
        <screen name="MultiMediathekSettings" position="center,center" size="560,330" title="tvalacarta - Settings">
    ...
    def __init__(self, session):
        ...
        self.setTitle("tvalacarta v%s - Settings" % config.plugins.tvalacarta.version.value)

        self.oldadultcontentvalue = config.plugins.tvalacarta.showadultcontent.value
        self.oldstoragepathvalue = config.plugins.tvalacarta.storagepath.value

        self.cfglist = []
        self.cfglist.append(getConfigListEntry(_("Thumbnail Caching:"), config.plugins.tvalacarta.imagecache))
        self.cfglist.append(getConfigListEntry(_("Thumbnail Scaling Mode:"), config.plugins.tvalacarta.imagescaling))
        self.cfglist.append(getConfigListEntry(_("Thumbnail Scaler:"), config.plugins.tvalacarta.imagescaler))
        self.cfglist.append(getConfigListEntry(_("Show Adult Content:"), config.plugins.tvalacarta.showadultcontent))
        #self.cfglist.append(getConfigListEntry(_("Show Secret Content:"), config.plugins.tvalacarta.showsecretcontent))
        self.cfglist.append(getConfigListEntry(_("Download Directory:"), config.plugins.tvalacarta.moviedir))
        self.cfglist.append(getConfigListEntry(_("Cache Folder:"), config.plugins.tvalacarta.storagepath))
        ConfigListScreen.__init__(self, self.cfglist, session)

    def keySave(self):
        config.plugins.tvalacarta.save()

        if config.ParentalControl.configured.value and config.plugins.tvalacarta.showadultcontent.value and config.plugins.tvalacarta.showadultcontent.value != self.oldadultcontentvalue:
        ...
        if not os_path.isdir(config.plugins.tvalacarta.storagepath.value):
            self.session.open(MessageBox, "The directory %s does not exist!" % config.plugins.tvalacarta.storagepath.value, MessageBox.TYPE_ERROR)
            return

        if config.plugins.tvalacarta.storagepath.value != self.oldstoragepathvalue:
            os_system("rm -rf "+self.oldstoragepathvalue+"/tvalacarta")
            os_system("mkdir -p "+config.plugins.tvalacarta.storagepath.value+"/tvalacarta")
            os_system("mkdir -p "+config.plugins.tvalacarta.storagepath.value+"/tvalacarta/images")
            os_system("mkdir -p "+config.plugins.tvalacarta.storagepath.value+"/tvalacarta/movies")
            os_system("mkdir -p "+config.plugins.tvalacarta.storagepath.value+"/tvalacarta/tmp")
    ...
    def pinEntered(self, result):
        if result is None:
            config.plugins.tvalacarta.showadultcontent.value = False
            config.plugins.tvalacarta.save()
        elif not result:
            config.plugins.tvalacarta.showadultcontent.value = False
            config.plugins.tvalacarta.save()
...
def main(session, **kwargs):
    print "[tvalacarta] main"
    session.open(Tvalacarta)

def Plugins(**kwargs):
    print "[tvalacarta] Plugins"
    return [
        PluginDescriptor(name = "tvalacarta", description = "tvalacarta 3.3.3 para Dreambox", icon="plugin-icon.png", where = PluginDescriptor.WHERE_PLUGINMENU, fnc = main),
        PluginDescriptor(name = "tvalacarta", description = "tvalacarta 3.3.3 para Dreambox", icon="plugin-icon.png", where = PluginDescriptor.WHERE_EXTENSIONSMENU, fnc=main)]
