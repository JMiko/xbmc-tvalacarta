...
if hasattr(sys, "argv") and len(sys.argv) > 1:
    ...
    try:
        ...
        if config.checkforupdates:
            ...
            globalLogFile.info("Buscando actualizaciones...")
            globalLogFile.info("Version actual="+versionactual)
            try:
                req = urllib2.Request("http://xbmc-tvalacarta.googlecode.com/files/pelisalacarta.ver")
                ...
                globalLogFile.info("Version nueva="+versionnueva)
                dialog = xbmcgui.Dialog()
                if (versionnueva > versionactual):
                    novedades = ""
                    posicion = link.find("whatsnew=")
                    globalLogFile.info("posicion=%d" % posicion)
                    novedades = ""
                    if posicion != -1:
                        novedades = link[ posicion+9 : ]
                        novedades = novedades.replace(r"\n","\n")
                    dialog.ok("Nueva versión "+versionnueva+" disponible",novedades)
            except:
                dialog = xbmcgui.Dialog()
