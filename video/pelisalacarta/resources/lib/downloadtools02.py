...
import xbmcplugin
import sys, os.path
import urllib,urllib2, filecmp, urlparse
...
def download(url):
	...
	# Saca el fichero de la URL
	nombrefichero =  urllib.unquote_plus( url.split("/")[-1] )
	xbmc.output("[downloadtools.py] nombrefichero="+nombrefichero)
	...
	#rutalocal = os.path.join( xbmc.translatePath( "special://home/plugins/video/pelisalacarta/" ) , nombretemporal )
	rutalocal = xbmc.translatePath( os.path.join( "special://temp/", nombretemporal ))
	xbmc.output("[downloadtools.py] rutalocal="+rutalocal)

	progreso.create( 'Pelisalacarta' , "Descargando vídeo..." , url , rutalocal )

	# Extrae host y query de la URL
	parsedurl = urlparse.urlparse(url)
	#xbmc.output("parsedurl=%s"  % parsedurl)

	try:
		host = parsedurl.netloc
	except:
		host = parsedurl[1]
	xbmc.output("host=%s" % host)
	...
	xbmc.output("query=%s" % query)

	resumeDL(rutalocal,(host,query))
...
def resumeDL(local, remote):
	...
	try:
		while 1:
			...
			xbmc.output("existSize=%d" % existSize) #sys.stdout.write('.')
			existSize = existSize + 8192
			percent = int( existSize / totalsize)
			progreso.update( percent , "Descargados %.2fMB" % (existSize / (1024*1024))  )
