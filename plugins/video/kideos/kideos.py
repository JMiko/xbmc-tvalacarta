...
def run():
	xbmc.output("[kideos.py] run")
	
	# Imprime en el log los parámetros de entrada
	xbmc.output("[kideos.py] sys.argv=%s" % str(sys.argv))
	
	# Crea el diccionario de parametros
	params = dict()
	if len(sys.argv)>=2 and len(sys.argv[2])>0:
		params = dict(part.split('=') for part in sys.argv[ 2 ][ 1: ].split('&'))
	xbmc.output("[kideos.py] params=%s" % str(params))
	...
	xbmc.output("[kideos.py] action="+action)
	...
	if (params.has_key("category")):
		...
		if params.has_key("channel"):
			...
			category = ""
	xbmc.output("[kideos.py] category="+category)
