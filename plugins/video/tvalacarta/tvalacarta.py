...
def run():
	xbmc.output("[tvalacarta.py] run")
	
	# Imprime en el log los parámetros de entrada
	xbmc.output("[tvalacarta.py] sys.argv=%s" % str(sys.argv))
	
	# Crea el diccionario de parametros
	params = dict()
	if len(sys.argv)>=2 and len(sys.argv[2])>0:
		params = dict(part.split('=') for part in sys.argv[ 2 ][ 1: ].split('&'))
	xbmc.output("[tvalacarta.py] params=%s" % str(params))
	...
	if (params.has_key("url")):
		...
		url=''
	xbmc.output("[tvalacarta.py] url="+url)
	...
	xbmc.output("[tvalacarta.py] action="+action)
	...
	if (params.has_key("category")):
		...
		if params.has_key("channel"):
			...
			category = ""
	xbmc.output("[tvalacarta.py] category="+category)
